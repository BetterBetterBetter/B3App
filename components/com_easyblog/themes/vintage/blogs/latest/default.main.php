	...
	<div class="eb-post-content">
		...
		<?php if (in_array($post->getType(), array('photo', 'standard', 'twitter', 'email', 'link'))) { ?>
			<div class="eb-post-body type-<?php echo $post->posttype; ?>">
				...
				<div class="eb-vintage-post-cover mb-10">
					<?php if (!$this->config->get('cover_crop', false)) { ?>
					<a href="<?php echo $post->getPermalink();?>" class="eb-post-image">
						<img src="<?php echo $post->getImage('medium');?>" alt="<?php echo $post->title;?>" itemprop="image" />
					</a>
					<?php } ?>

					<?php if ($this->config->get('cover_crop', false)) { ?>
					<a href="<?php echo $post->getPermalink();?>" class="eb-post-image-cover" style="background-image: url('<?php echo $post->getImage('medium');?>');"></a>
					<?php } ?>
				</div>
