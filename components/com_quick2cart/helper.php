...
class Comquick2cartHelper
{
	 ...
	 *      [item_name] => Pizza Base - L'exclusif
            ...
            [description] => Array
                (
                    [data] => Pita Bread - L'Exclusif - 150 gmPita Bread - L'Exclusif - 150 gmPita Bread - L'Exclusif - 150 gmPita Bread - L'Exclusif - 150 gmPita Bread - L'Exclusif - 150 gm
                )
	...
	public function getViewpath($viewname, $layout = "", $searchTmpPath = 'SITE', $useViewpath = 'SITE')
	{
		...
		$overide_basepath = $override = $searchTmpPath . '/templates/' . $app->getTemplate() . '/html/com_quick2cart/' . $viewname . '/' . $layoutname;
		...
		{
			return $view = $useViewpath . '/components/com_quick2cart/views' . DS . $viewname . '/tmpl' . DS . $layoutname;
		}
	}
	 ...
	 * @param   STRING  $singlemail      singlemail
	 * @param   STRING  $attachmentPath  attachmentPath
	 *
	 * @return  boolean
	 */
	public function sendmail($recipient, $subject, $body, $bcc_string = '', $singlemail = 1, $attachmentPath = "")
	{
...
//print $body; die;
	...
	*/
	...
	public function updateApplicationXml($path)
	{
		...
		$find = JPATH_SITE . DS . 'components' . DS . 'com_zoo';
	...
	public function uploadImage($file_field, $oldImgPath = "")
	{
		...
		// Always use constants when making file paths, to avoid the possibilty of remote file inclusion
		$upload_path_folder       = JPATH_SITE . DS . 'images' . DS . 'quick2cart';
		$image_upload_path_for_db = 'images' . DS . 'quick2cart';
	...
	/**
		 * To override the icon set for backend / front end view. Check whether icon define file is add in template. If added then use override file else use compoent's file
		 *
