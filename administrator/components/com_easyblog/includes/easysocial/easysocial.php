...
class EasyBlogEasySocial extends EasyBlog
{
	...
	public function createBlogStream($blog, $isNew)
	{
		...
		$esClusterType = array(EASYBLOG_POST_SOURCE_EASYSOCIAL_EVENT, EASYBLOG_POST_SOURCE_EASYSOCIAL_GROUP);
		...
		if ($contribution) {
			...
			if (in_array($contribution->type, $esClusterType)) {
				...
				if ($contribution->type == EASYBLOG_POST_SOURCE_EASYSOCIAL_EVENT) {
					$clusterType = 'event';
				}

				$template->setCluster($contribution->id, $clusterType);
